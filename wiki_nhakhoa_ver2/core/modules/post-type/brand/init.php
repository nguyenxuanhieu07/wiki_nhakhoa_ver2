...
if (!function_exists('get_table_service')) {
	function get_table_service()
	{
		$post_id = $_POST['post_id'];
		$post_term = $_POST['id_term'];
		$html = '';
		$list_service = rwmb_meta('brand-table', '', $post_id) ? rwmb_meta('brand-table', '', $post_id) : '';
		//echo '<pre>';
		//	print_r($_POST);
		//	echo '</pre>';
		ob_start();
			...
			if ($post_term == $term_id) {
				...
				<table>
					<tbody>
						...
						<?php
						foreach ($value['option-table'] as $key => $value) {
							?>
							<tr>
								<td>
									<?php echo $key + 1; ?>
...
add_action('wp_ajax_get_table_service', 'get_table_service');
add_action('wp_ajax_nopriv_get_table_service', 'get_table_service');
